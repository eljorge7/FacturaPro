...
// 1. State
const state_old = "  const [isSolarModalOpen, setIsSolarModalOpen] = useState(false);";
const state_new = `  const [isSolarModalOpen, setIsSolarModalOpen] = useState(false);
...
// 2. Form state
const form_old = `      costoKwp: 18000,
      tarifaCfe: 2.20`;
const form_new = `      costoKwp: 18000,
      costoInstalacion: 15000,
      tarifaCfe: 2.20`;
content = content.replace(form_old, form_new);

// 3. Equipamiento UI
const ui_old = `<div className="col-span-2">
...
const ui_new = `<div className="col-span-2 p-3 bg-slate-100 rounded-lg border border-slate-200">
...
// 5. ROI Logic
const roi_old = "const inversionTotal = potenciaInstalada * solarForm.costoKwp;";
const roi_new = `const costoPaneles = (selectedPanel?.finalPrice || 0) * numPaneles;
...
const close_old = `<button onClick={() => setIsSyscomModalOpen(false)} className="p-2 hover:bg-purple-100 rounded-full text-slate-500">`;
const close_new = `<button onClick={() => { setIsSyscomModalOpen(false); setSyscomSelectMode(null); }} className="p-2 hover:bg-purple-100 rounded-full text-slate-500">`;
content = content.replace(close_old, close_new);

// 7. Syscom Select Logic
const syscom_select_old = `                      let finalPrice = prod.price;
                      if (currency === 'MXN') {
...
const syscom_select_new = `                      let finalPrice = prod.price;
                      if (currency === 'MXN') {
                         finalPrice = prod.price * exchangeRate;
                      }
