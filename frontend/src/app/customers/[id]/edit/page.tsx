...
export default function EditCustomerPage() {
  ...
  useEffect(() => {
     if (!customerId || !activeTenantId) return;
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Actualice los datos fiscales y de contacto principales.</p>
          </div>
        </div>
        <div className="flex gap-3">
           ...
           >
             {isSaving ? <span className="animate-pulse">Actualizando...</span> : <><Save className="w-4 h-4"/> Actualizar</>}
           </button>
        </div>
      </div>

      <div className="max-w-4xl mx-auto w-full mt-8 bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden relative">
         
         {isLoading && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex items-center justify-center">
               <span className="animate-pulse font-bold text-slate-400">Cargando datos del cliente...</span>
            </div>
         )}
