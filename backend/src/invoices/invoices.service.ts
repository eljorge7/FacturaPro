...
export class InvoicesService {
  ...
  async create(createInvoiceDto: any) {
    ...
    if ((tenantObj as any).subscriptionEndsAt && new Date() > new Date((tenantObj as any).subscriptionEndsAt)) {
        throw new BadRequestException('Tu periodo de prueba o suscripción ha expirado. Por favor, realiza Upgrade de tu plan para seguir facturando.');
    }

    // Validación de SaaS Timbres ANTES de computar nada
    if (!isDraft && tenantObj.availableStamps !== -1) {
      if (tenantObj.availableStamps <= 0) {
        throw new BadRequestException('Límite de Timbres Agotado. Por favor adquiera más timbres o mejore su plan.');
      }
    }

    // 2. Obtener o Auto-crear el TaxProfile del Tenant para robustez M2M
    ...
    if (!customer) throw new NotFoundException('Cliente receptor no encontrado');
    ...
    // Configuración Base de Entidad Temporal
    ...
    if (!isDraft) {
        // --- 4.5. Generar XML Local y Sellarlo Digitalmente ---
        const { xml } = await this.xmlGenerator.generate(partialInvoice, taxProfile);

        // --- 4.6. Enviar a PAC para Timbrado ---
    ...
    // 5. Crear la Factura y Descontar Timbre (Transacción)
    const [invoice] = await this.prisma.$transaction([
      this.prisma.invoice.create({
        data: {
          ...
          customerId,
          invoiceNumber,
          status: createInvoiceDto.status || 'TIMBRADA', // TIMBRADA o DRAFT
          satUuid,
          paymentMethod,
          paymentForm,
          cfdiUse,
          ...
          customFields: (createInvoiceDto as any).customFields,
          items: {
  ...
  async getStats(tenantId: string) {
    ...
    // Data para gráfico  
    // Por hacer simple, devolver 6 meses aleatorios o caculados.
       ...
       allInvoices.forEach(inv => {
          if (inv.status !== 'CANCELADA') {
             ...
             if (idt.getMonth() === m && idt.getFullYear() === y) {
                 if (inv.paymentMethod === '99') sumCred += inv.total;
                 else sum += inv.total;
             }
          }
       });
    ...
    return {
       ...
       subscriptionEndsAt: (tenant as any)?.subscriptionEndsAt || null,
  ...
  async registerPayment(id: string, payload: { amount: number; paymentDate: string; paymentMethod: string; reference?: string; notes?: string }) {
     const invoice = await this.findOne(id);
     
     if (invoice.status === 'CANCELADA') {
        throw new BadRequestException('No se pueden registrar pagos a una factura cancelada.');
     }
     ...
     // Threshold para considerar pagada:
     // Debido a fluctuaciones de decimales, usamos un delta pequeño
     ...
     const result = await this.prisma.$transaction(async (tx) => {
        ...
        // 2. Actualizar estado de Factura si corresponde
  ...
  async cancelFiscal(id: string, motive: string, substitutionUuid?: string) {
    ...
    if (invoice.status === 'CANCELADA') throw new BadRequestException('La factura ya está cancelada');
    if (invoice.status === 'DRAFT' || !invoice.satUuid) {
       // Si no fue timbrada, cancelamos localmente directo
       return this.cancel(id);
    }

    const tp = invoice.taxProfile;
    
    // Si estamos en entorno de pruebas, usamos los certificados de prueba del SAT
    ...
    // Helper to read test files if missing (SANDBOX environment)
    if (!b64Cer || !b64Key || !csdPassword) {
       ...
       if (fs.existsSync(cerPath) && fs.existsSync(keyPath)) {
          ...
          csdPassword = '12345678a'; // Test SAT password
          rfcEmisor = 'EKU9003173C9'; // Default SAT Test RFC
       } else {
          throw new BadRequestException('El perfil fiscal no tiene sus archivos CSD configurados y no hay archivos de prueba disponibles.');
       }
    }

    const cleanBase64 = (str: string | null | undefined) => str ? str.replace(/^data:[^;]+;base64,/, "") : "";

    // 1. Solicitar PAC env = SANDBOX for testing
    ...
    }, 'SANDBOX'); // Todo: make it dependent on env

    // 2. Si es exitoso, marcar en BD
    return this.prisma.invoice.update({
      where: { id },
      data: { status: 'CANCELADA' }
    });
  ...
  async stamp(id: string) {
    const invoice = await this.findOne(id);
    if (invoice.status !== 'DRAFT') {
      throw new BadRequestException('Solo se pueden timbrar facturas en borrador.');
    }
    ...
    // --- Generar XML Local y Sellarlo Digitalmente ---
  ...
  async sendWhatsapp(id: string, phone: string) {
    ...
    // Asumimos un PUBLIC_URL global o caemos a localhost para el link de PDF
  ...
  async getArReport(tenantId: string) {
    ...
    // 1. Reclutar todos los pagos históricos asociados al Tenant (Ingresos a favor)
