...
let PdfService = class PdfService {
    ...
    drawTemplateHeader(doc, data, isQuote, template) {
        ...
        if (template === 'Ticket POS Termal') {
            if (logoPath)
                doc.image(logoPath, (doc.page.width - logoWidth) / 2, 20, { width: logoWidth, align: 'center' });
            else
                doc.fillColor(bColor).fontSize(14).font(fBold).text(data.taxProfile?.legalName || 'Empresa', 10, 20, { align: 'center', width: doc.page.width - 20 });
            doc.moveDown(1);
            ...
            if (logoPath)
                ...
                doc.fillColor('#ffffff').fontSize(22).font(fBold).text(data.taxProfile?.legalName || 'Empresa', 50, 40);
            ...
            if (logoPath)
                doc.image(logoPath, 50, 30, { width: logoWidth, align: 'left' });
            else
                doc.fillColor('#ffffff').fontSize(24).font(fBold).text(data.taxProfile?.legalName || 'Empresa', 50, 40, { width: 280 });
            ...
            if (logoPath)
                doc.image(logoPath, 450, 45, { width: logoWidth, align: 'right' });
            ...
            if (logoPath)
                doc.image(logoPath, 60, 55, { width: logoWidth * 0.8, align: 'left' });
    ...
    drawFooter(doc, data, isQuote, template) {
        if (template === 'Ticket POS Termal') {
        ...
        if (isQuote) {
            ...
            if (data.xmlContent) {
                ...
                if (mEmisor)
                    selloEmisor = mEmisor[1].substring(0, 80) + '...';
            }
