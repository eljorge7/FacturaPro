...
let PublicStoreService = PublicStoreService_1 = class PublicStoreService {
    ...
    async getSyscomToken(tenant) {
        if (!tenant.syscomClientId || !tenant.syscomClientSecret)
            return null;
        ...
        try {
            const response = await axios_1.default.post('https://developers.syscom.mx/oauth/token', new URLSearchParams({
            ...
            try {
                ...
                if (res.data && res.data.productos) {
                    syscomProducts = res.data.productos.map((p) => {
                        ...
                        const isRF = p.categorias?.some((cat) => cat.nombre && /radiocomunicaci|duplexer|repetidor|antena/i.test(cat.nombre));
                        return {
                            id: p.producto_id,
                            title: p.titulo,
                            model: p.modelo,
    ...
    async getProductDetails(slug, id) {
        ...
        const token = await this.getSyscomToken(tenant);
        if (!token)
            throw new common_1.NotFoundException('Product not found or syscom not configured');
        try {
            const res = await axios_1.default.get(`https://developers.syscom.mx/api/v1/productos/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            const p = res.data;
            const specialPrice = parseFloat(p.precios?.precio_especial || '0');
    ...
    async generatePaymentLink(slug, id) {
        ...
        if (order.status !== 'PENDING')
            throw new common_1.BadRequestException('Order is already paid or processed');
        try {
            const url = await this.createPreference(tenant, order);
            ...
            throw new common_1.BadRequestException('No se pudo generar el link de pago');
        }
    }
    async createPreference(tenant, order) {
        ...
        const res = await axios_1.default.post('https://api.mercadopago.com/checkout/preferences', body, {
            headers: {
...
exports.PublicStoreService = PublicStoreService = PublicStoreService_1 = __decorate([
